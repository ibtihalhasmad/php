...
if ($signed === $data['x_signature']) {
    if ($paidstatus == "Success"){ 
        ...
        $sqpupdatecart = "UPDATE `tbl_subcart` SET `cart_status`='paid',`receipt_id`='$receiptid' WHERE user_email='$email' AND cart_status IS NULL";
        if ($conn->query($sqlinsertpayment) && $conn->query($sqpupdatecart)){
            $sqlselectcart="SELECT * FROM tbl_subcart WHERE receipt_id = '$receiptid'";
            $result = $conn->query($sqlselectcart);
            if ($result->num_rows > 0) {
                while ($rows = $result->fetch_assoc()) {
                    $prid = $rows['product_id'];
                    $prqty = $rows['cart_qty'];
                    $updateproductsqty = "UPDATE tbl_subjects set subject_qty = (subject_qty-$subqty) WHERE product_id = '$prid'";
